...
function Navbar() {
  ...
  return (
<nav className="fixed top-0 left-0 z-50 bg-white shadow">
...
>
  ...
  {/* Menu */}
  <div
    className={`absolute top-0 right-0 h-full w-2/3 max-w-sm bg-[#062b32] text-white flex flex-col overflow-y-auto shadow-2xl
      ...
      (user ? (
        <div className="flex flex-col gap-4 px-5 py-4">
          <Link
            to="/admin/dashboard"
            onClick={() => setMenuOpen(false)}
            className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-orange-500 transition"
          >
            <div className="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold">
              {user.name.charAt(0).toUpperCase()}
            </div>
          ...
          <button
            onClick={handleLogout}
            className="bg-red-600 mx-5 py-2 rounded-lg hover:bg-red-700"
          >
            Logout
          </button>
        </div>
        ...
        <Link
          to="/admin/login"
          onClick={() => setMenuOpen(false)}
          className="mx-5 my-4 text-center bg-blue-600 rounded py-2 hover:bg-white hover:text-blue-600"
        >
          Login
        </Link>
      ))}
